...
struct PCGSolver
{
    PCGSolver(void)
    {
        set_solver_parameters(1e-10, 150000, 0.97, 0.25);
    }
	...
	//std vector rhs, tamaño de (se me ha olvidado)
	//resolver presiones con tipo double mejor que con tipo float
    bool solve(const SparseMatrix<T> &matrix, const std::vector<T> &rhs, std::vector<T> &result, T &residual_out, int &iterations_out )
    {
...
#else
        zero(r);
        multiply(matrix, result, z);
        BLAS::add_scaled<T>(-1.0,z,r);
        BLAS::add_scaled<T>(1.0,rhs,r);
#endif
        ...
        for(iteration=0; iteration<max_iterations; ++iteration){
            ...
            BLAS::add_scaled<T>(alpha, s, result);
            BLAS::add_scaled<T>(-alpha, z, r);
            residual_out=BLAS::abs_max(r);
            if(residual_out<=tol) {
            ...
            BLAS::add_scaled<T>(beta, s, z); s.swap(z); // s=beta*s+z
            rho=rho_new;
        }
